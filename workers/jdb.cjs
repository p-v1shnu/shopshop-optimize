process.env.LOGGER_APP_NAME = 'shopshop-jdb-worker'
...
async function main() {
  const pubnub = new PubNub({
    subscribeKey : process.env.JDB_QR_PUBNUB_SUBKEY,
    ssl          : true,
    uuid         : process.env.JDB_QR_PUBNUB_USERID,
  })

  const pubnubChannels = [
    `mcid-${process.env.JDB_QR_MC_ID}-${process.env.JDB_QR_MC_CODE}`,
  ]
  ...
  pubnub.addListener({
    ...
    message: async (messageEvent) => {
      const payment = JSON.parse(messageEvent.message)
      const billNumber = payment.billNumber
      ...
      if (billNumber.startsWith(process.env.PAYMENT_UUID_PREFIX)) {
        try {
          ...
          const { data } = await axios.post(process.env.JDB_QR_WEBHOOK_URL, payment, {
            validateStatus: () => true,
          })
  ...
  logger.info({ msg: 'PubNub JDB worker is running' })
}
