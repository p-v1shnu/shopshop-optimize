<div class="flex flex-1 flex-col overflow-y-scroll">
  ...
  <main class="h-full overflow-y-scroll p-4 space-y-4">
    ...
    <div class="border border-zinc-300 rounded-2xl p-4 text-center text-sm font-bold space-y-4 bg-white">
      <div>{{ config('custom.bcel_qr_mc_name') }}</div>
      <div class="mx-auto w-max rounded-lg bg-white p-2 shadow drop-shadow-lg relative">
        <img src="{{ \App\Utils\AppUtil::asset('resources/images/icon-bcelone.svg') }}" width="24" height="24" class="mx-auto shadow rounded bg-white p-0.5 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
  ...
  <footer class="right-0 bottom-0 left-0 bg-white shadow-t-lg">
    ...
    <div class="m-4">
      <button type="button" class="flex w-full items-center justify-center rounded-2xl border border-gray-300 bg-gray-50 p-2 space-x-4" onclick="window.open('onepay://qr/{{ $order->generate_qr_response['data']['emv'] }}', '_blank')">
        <img src="{{ \App\Utils\AppUtil::asset('resources/images/icon-bcelone.svg') }}" class="rounded shadow size-10">
        <span class="font-semibold">ຈ່າຍເງິນຜ່ານ OnePay</span>
      </button>
    </div>
